...
Begin VB.Form F_Brand 
   ...
   Begin VB.Image Image1 
      Height          =   6090
      Left            =   0
      Picture         =   "F_Brand.frx":111E
      Stretch         =   -1  'True
      Top             =   0
      Width           =   5910
   End
End
Attribute VB_Name = "F_Brand"
...
If intID = 0 Then
  Call psubClear
Else
     If MsgBox("Are you sure you want to delete this brand ?", vbYesNo + vbQuestion, SystemTitle) = vbYes Then
        
        strSQL = ""
        strSQL = strSQL & " delete from brand"
        strSQL = strSQL & " Where id=" & intID
        
        clsConnect.DBConnect.Execute (strSQL)
        MsgBox "Brand deleted successfully.", vbInformation, SystemTitle
        Call psubClear
        Call psubGetData
...
Private Function pfBlnAlreadyExist(blnUpdate As Boolean) As Boolean
  Dim objAlreadyExist As Object
        strSQL = ""
        strSQL = strSQL & " select * from  brand where name =" & pfstrQuote(txtName.Text)
        If blnUpdate Then
        ...
        If Not objAlreadyExist.EOF Then
            ...
            txtName.SetFocus
            MsgBox "Brand already exist.", vbCritical, SystemTitle
            Exit Function
        End If
End Function
Private Sub cmdSave_Click()
    ...
    If pfBlnAlreadyExist(False) Then Exit Sub
        If MsgBox("Are you sure you want to add this brand ?", vbYesNo + vbQuestion, SystemTitle) = vbYes Then
            strSQL = ""
            strSQL = strSQL & " Insert into brand"
            strSQL = strSQL & " (name)"
            strSQL = strSQL & " values ( " & pfstrQuote(txtName.Text) & ")"
            
            clsConnect.DBConnect.Execute (strSQL)
            MsgBox "Brand added successfully.", vbInformation, SystemTitle
    ...
    Else
        ...
        If MsgBox("Are you sure you want to update this brand ?", vbYesNo + vbQuestion, SystemTitle) = vbYes Then
        
            strSQL = ""
            strSQL = strSQL & " Update brand"
            strSQL = strSQL & " SET name=" & pfstrQuote(txtName.Text)
            strSQL = strSQL & " where id=" & intID
            
            clsConnect.DBConnect.Execute (strSQL)
            MsgBox "Brand update successfully.", vbInformation, SystemTitle
