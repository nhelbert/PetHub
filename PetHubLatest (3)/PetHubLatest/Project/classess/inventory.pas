...
begin
  ...
  try
    Form3.inventorymysql.Connected:= true;
    Form3.inventoryquery.SQL.Text := 'SELECT * FROM inventory WHERE deleted = 0 AND `expirationDate` >= '+ QuotedStr(dateNow);
...
begin
  ...
  try
    Form3.inventorymysql.Connected:= true;
         ...
         Form3.inventoryquery.SQL.Text := 'SELECT * FROM inventory WHERE (productName LIKE '+QuotedStr(productNameSrch+'%')+'or productID LIKE '+QuotedStr(productNameSrch+'%')+' or productType LIKE '+QuotedStr(productNameSrch+'%')+' or productBrand LIKE '+QuotedStr(productNameSrch+'%')+' or expirationDate LIKE '+QuotedStr(productNameSrch+'%')+') AND deleted = 0 AND `expirationDate` >= '+ QuotedStr(dateNow);
